...
def normalize_client_code(client_code):
    ...
    # Reverse lookup - if they sent a name, convert to code
    name_to_code = {
        'one nz': 'ONE',
...
def build_completed_section(completed_projects):
    ...
    items = "".join([
        f"<li style='margin-bottom: 8px;'><strong style='color: #ED1C24;'>{p['job_number']}</strong> â€” {p['job_name']} â€” {p['description']}</li>"
...
def build_wip_email(client_name, projects, completed_projects, header_url=''):
    ...
    if header_url:
        ...
        header_content = f'''<span style="font-size: 28px; font-weight: bold; color: #ED1C24;">HUNCH â€” WIP</span>'''
